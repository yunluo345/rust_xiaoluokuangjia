...
**已落地（2026-02-28）**:
- 新增管理员接口：`POST /jiekou/ai/diaoduqi/gengxin`
- 支持在线更新：`quanju_shangxian`、`paidui_chaoshi_miao`
- 更新后立即返回当前调度器状态与排队超时配置
...
**已落地（2026-02-28）**:
- 在 `openaizhuti.rs` 引入统一总超时窗口：`总超时 = HTTP超时 + 调度器排队超时`
- `fasong_qingqiu` 改为按剩余时间控制排队、单次HTTP请求、重试退避等待
- `putongqingqiu_neibu` 的响应体限流重试也复用同一截止时间，避免跨层级超时叠加失控
...
**已落地（2026-02-28）**:
- 新增监控接口：`GET /jiekou/ai/diaoduqi/jiankong`
- 输出 Prometheus 文本指标：
        ...
        → futures::join!(标题, 摘要, 思维导图, 关系)  # 子步骤并发
          → ai_putongqingqiu_wenben(chaoshi=30-120, chongshi=3)
            → openaizhuti::putongqingqiu()
              → fasong_qingqiu()
                → timeout(剩余总超时, diaoduqi::huoqu_xukezheng())  # 🔒 获取许可证
